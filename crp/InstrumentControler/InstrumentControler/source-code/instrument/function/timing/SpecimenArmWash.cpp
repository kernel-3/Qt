...
函数中，调用 Action::execute 函数失败！"),(COleDateTime::GetCurrentTime()).
	...
	if(STATUS_ENABLED==status){
		...
		if((((Instrument::instance())->get_components())->
			...
			CString log(_T(""));
			log.Format(_T("Error %s（%ld）： SpecimenArmWash::execute \
函数中，调用 SpecimenArm::wash 函数失败！"),(COleDateTime::GetCurrentTime()).
				Format(_T("%Y-%m-%d %H:%M:%S")),timeGetTime());
			Logs::instance()->write(Logs::NAME_ERROR,log);
			//2-1-3.程序运行到此直接返回错误。
			return(-2);
		...
		//2-2.书写日志。
		CString log(_T(""));
		log.Format(_T("Event %s（%ld）： 样本臂开始清洗。"),
...
函数中，调用 Action::complete 函数失败！"),(COleDateTime::GetCurrentTime()).
	...
	if(STATUS_ENABLED==status){
		...
		if(SpecimenArm::FLAG_IDLE==(((Instrument::instance())->
			...
			//2-1-1.输出日志。
			CString log(_T(""));
			log.Format(_T("Event %s（%ld）： 样本臂清洗成功完成。"),
				(COleDateTime::GetCurrentTime()).Format(
				_T("%Y-%m-%d %H:%M:%S")),timeGetTime());
			Logs::instance()->write(Logs::NAME_EVENT,log);
			//2-1-2.程序运行到此直接成功返回。
		...
		//2-2.如果洗样本尚未完成。
			...
			if(get_defeats()>=MAX_DEFEATS){
				...
				CString log(_T(""));
				log.Format(_T("Error %s（%ld）： SpecimenArmWash::complete \
函数中，检测到样本臂清洗严重超时！"),(COleDateTime::GetCurrentTime()).Format(_T(
			...
			//2-2-3.如果当前错误次数尚未抵达上限。
				...
				//b.书写错误日志。
				CString log(_T(""));
				log.Format(_T("Warning %s（%ld）： 样本臂清洗时发生超时！"),
					(COleDateTime::GetCurrentTime()).Format(
					_T("%Y-%m-%d %H:%M:%S")),timeGetTime());
				Logs::instance()->write(Logs::NAME_EVENT,log);
				Logs::instance()->write(Logs::NAME_ERROR,log);
