...
函数中，调用 Action::execute 函数失败！"),(COleDateTime::GetCurrentTime()).
	...
	if(STATUS_ENABLED==status){
		...
		if((((Instrument::instance())->get_components())->
			...
			CString log(_T(""));
			log.Format(_T("Error %s（%ld）： TakeTubeOutOfDetectionDisc::execute \
函数中，调用 TransportTong::take_tube_out_of_detection_disc 函数失败！"),
		...
		//2-2.书写日志。
		CString log(_T(""));
		log.Format(_T("Event %s（%ld）： 转运抓手从检测盘开始取管。"),
...
函数中，调用 Action::complete 函数失败！"),(COleDateTime::GetCurrentTime()).Format(
	...
	if(STATUS_ENABLED==status){
		...
		if(TransportTong::FLAG_IDLE==(((Instrument::instance())->
			...
			//2-1-1.输出日志。
			CString log(_T(""));
			log.Format(_T("Event %s（%ld）： 转运抓手从检测盘取管完成。"),
				(COleDateTime::GetCurrentTime()).Format(
				_T("%Y-%m-%d %H:%M:%S")),timeGetTime());
			Logs::instance()->write(Logs::NAME_EVENT,log);
			//2-1-2.程序运行到此直接成功返回。
		...
		//2-2.如果洗试剂尚未完成。
			...
			if(get_defeats()>=MAX_DEFEATS){
				...
				CString log(_T(""));
				log.Format(_T("Error %s（%ld）： TakeTubeOutOfDetectionDisc::complete \
函数中，检测到转运抓手从检测盘取管严重超时！"),(COleDateTime::GetCurrentTime()).Format(_T(
			...
			//2-2-3.如果当前错误次数尚未抵达上限。
				...
				//b.书写错误日志。
				CString log(_T(""));
				log.Format(_T("Warning %s（%ld）： 转运抓手从检测盘取管时发生超时！"),
					(COleDateTime::GetCurrentTime()).Format(
					_T("%Y-%m-%d %H:%M:%S")),timeGetTime());
				Logs::instance()->write(Logs::NAME_EVENT,log);
				Logs::instance()->write(Logs::NAME_ERROR,log);
