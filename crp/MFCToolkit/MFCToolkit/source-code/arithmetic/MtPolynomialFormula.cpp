...
int MtPolynomialFormula::calculate_x(const DMat &parameters,
	const double y,const double target_x,
	const int target_x_relative_position,double &x){
	//1.声明当前操作所需变量。
	...
	//3.计算新的多项式系数。
	DMat parameters_new;
	if(parameters_new.copy(parameters)<0){
		return(-2);
	}
	parameters_new.set_element(rows-1,0,
		parameters_new.get_element(rows-1,0)-y);
	//4.计算多项式的根，并且判断计算是否成功。
	if(MtSolvingPolynomial::solve(parameters_new,target_x,
		target_x_relative_position,x)<0){
		return(-3);
	}
	//5.程序运行到此成功返回。
	return(0);
...
int MtPolynomialFormula::calculate_maximum_relative_deviation(
	...
	for(unsigned int i=b_i;i!=size;++i){
		...
		double c_x=0.0;
		if(calculate_x(parameters,y,get_x(i),0,c_x)<0){
			return(-4);
		}
