...
int MtSolvingPolynomial::solve(const DMat &coefficient,
	const double target_value,const int target_value_relative_position,
	double &root){
	...
	if(3==hightest_power){
		if(cardano_method(coefficient,target_value,
			target_value_relative_position,root)<0){
			return(-2);
		}else{
			return(0);
		}
...
int MtSolvingPolynomial::cardano_method(
	...
	if(abs(D)<TINY){
		...
		//5-4.筛选求解的根，找出最符合要求的根。
		//a.声明当前操作所需变量。
		double root_t=0.0;
		//b.如果当前靶值与待测值重合。
		if(0==target_value_relative_position){
			root_t=x1;
			if(abs(root_t-target_value)>abs(x2-target_value)){
				root_t=x2;
			}
		}
		//c.如果靶值小于待测点。
		else if(target_value_relative_position<0){
			if(x1>=target_value){
				root_t=x1;
			}else{
				root_t=x2;
	...
	//6.如果当前存在一个实根以及两个虚根。
		...
		if(0==target_value_relative_position){
			root_t=x1;
			if(abs(root_t-target_value)>abs(x2_r-target_value)){
				root_t=x2_r;
			}
			if(abs(root_t-target_value)>abs(x3_r-target_value)){
				root_t=x3_r;
			}
		}
		//c.如果靶值小于待测点。
	...
	//7.如果当前存在三个实根，并且三个实根都不相等。
		...
		if(0==target_value_relative_position){
			root_t=x1.real();
			if(abs(root_t-target_value)>abs(x2.real()-target_value)){
				root_t=x2.real();
			}
			if(abs(root_t-target_value)>abs(x3.real()-target_value)){
				root_t=x3.real();
			}
		}
		//c.如果靶值小于待测点。
		else if(target_value_relative_position<0){
			if(x1.real()>=target_value){
				root_t=x1.real();
			}else if(x2.real()>=target_value){
				root_t=x2.real();
			}else{
				root_t=x3.real();
			}
