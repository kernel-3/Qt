// source-code/ui/utility/DialogUtility.cpp : implementation file
//
#include"stdafx.h"
...
IMPLEMENT_DYNAMIC(DialogUtility,DialogBase)
 ...
 * return: --
 */
DialogUtility::DialogUtility(CWnd *pParent/*=NULL*/)
	:DialogBase(DialogUtility::IDD,pParent)
	...
	_page_dialogs[0]=new DialogSystem;
	_page_dialogs[3]=new DialogApplication;
}
 ...
 * return: --
 */
void DialogUtility::DoDataExchange(CDataExchange *pDX){
	DialogBase::DoDataExchange(pDX);
	for(unsigned int index=0;index!=PAGES;++index){
		DDX_Control(pDX,IDC_BUTTON_SYSTEM+index,_page_buttons[index]);
	}
}

BEGIN_MESSAGE_MAP(DialogUtility,DialogBase)
	ON_WM_SIZE()
	ON_WM_DESTROY()
	ON_COMMAND_RANGE(IDC_BUTTON_SYSTEM,
		IDC_BUTTON_COMMUNICATION_TRACE,&DialogUtility::OnPageButtonClicked)
END_MESSAGE_MAP()

//DialogUtility message handlers
 ...
 * return: 此函数一直返回TRUE。
 */
BOOL DialogUtility::OnInitDialog(){
	...
	init_controls();
	//2.显示当前的子对话框。
	OnPageButtonClicked(IDC_BUTTON_SYSTEM);
	/////////////////////////////////////////////////////////////////
 ...
 * return: --
 */
void DialogUtility::OnSize(UINT nType,int cx,int cy){
 ...
 * return: --
 */
void DialogUtility::OnPageButtonClicked(const unsigned int ID){
	//1.计算当前点击按钮的下标，并且判断计算是否成功。
	const unsigned int index=ID-IDC_BUTTON_SYSTEM;
	if(index>=PAGES){
		return;
	}
	...
	if(0==index){
		dialog_id=DialogSystem::IDD;
	}else if(3==index){
		dialog_id=DialogApplication::IDD;
	}else{
		return;
	}
	//4-2.创建页面，并且判断创建是否成功。
	...
	if(_main_layout->set_item_window(1,_page_dialogs[index],500)<0){
		return;
	}
 ...
 * return: --
 */
void DialogUtility::init_controls(){
	//1.声明初始化当前操作所需变量。
	CRect rect;
	GetClientRect(rect);
	...
	//3-1.初始化子布局1。
	for(unsigned int index=0;index!=PAGES;++index){
		_child_layout_1->add_window(&_page_buttons[index],1);
	}
	//3-2.初始化主布局。
