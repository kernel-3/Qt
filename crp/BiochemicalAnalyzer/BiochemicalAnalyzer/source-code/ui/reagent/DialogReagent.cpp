// source-code/ui/reagent/DialogReagent.cpp : implementation file
//

#include"stdafx.h"
#include"DialogReagent.h"
...
//DialogReagent dialog
IMPLEMENT_DYNAMIC(DialogReagent,DialogBase)
 ...
 * return: --
 */
DialogReagent::DialogReagent(CWnd *pParent/*=NULL*/)
	:DialogBase(DialogReagent::IDD,pParent)
	...
	_page_dialogs[0]=new DialogReagentSetting;
}
 ...
 * return: --
 */
void DialogReagent::DoDataExchange(CDataExchange *pDX){
	DialogBase::DoDataExchange(pDX);
	for(unsigned int index=0;index!=PAGES;++index){
		DDX_Control(pDX,IDC_BUTTON_REAGENT_SETTING+index,_page_buttons[index]);
	}
}
 ...
 * return: 此函数一直返回TRUE。
 */
BOOL DialogReagent::OnInitDialog(){
...
BEGIN_MESSAGE_MAP(DialogReagent,DialogBase)
	ON_WM_SIZE()
	ON_WM_DESTROY()
	ON_COMMAND_RANGE(IDC_BUTTON_REAGENT_SETTING,IDC_BUTTON_REAGENT_SETTING,&DialogReagent::OnPageButtonClicked)
END_MESSAGE_MAP()

//DialogReagent message handlers
 ...
 * return: --
 */
void DialogReagent::OnSize(UINT nType,int cx,int cy){
	DialogBase::OnSize(nType,cx,cy);
	// TODO: Add your message handler code here
	//1.调整当前布局尺寸。
	_main_layout->resize(cx,cy);
	//2.显示当前的子对话框。
	OnPageButtonClicked(IDC_BUTTON_REAGENT_SETTING);
}
 ...
 * return: --
 */
void DialogReagent::OnPageButtonClicked(const unsigned int ID){
	//1.计算当前点击按钮的下标，并且判断计算是否成功。
	const unsigned int index=ID-IDC_BUTTON_REAGENT_SETTING;
	if(index>=PAGES){
		return;
	}
	...
	if(0==index){
		dialog_id=DialogReagentSetting::IDD;
	}else{
		return;
	}
	//4-2.创建页面，并且判断创建是否成功。
 ...
 * return: --
 */
void DialogReagent::init_controls(){
	//1.声明初始化当前操作所需变量。
	CRect rect;
	GetClientRect(rect);
