...
//初始化静态变量
const double MtLMFitting::V_1 = 2.;
const double MtLMFitting::V_2 = 200.;
...
int MtLMFitting::fit() {
	...
	for (; index != ITERATIONS; ++index) {
		...
		auto ret = calculate_lambda1(beta, lambda, s0, v);
		if (ret < 0 || _isnan(lambda) || lambda > MAX_LAMBDA)
			break;
		if (ret)
		{
			lambda *= V_1;
			...
			_formula.calculate_correlation_coefficient(beta, temp2) >= 0))
		{
			...
			if (index == 0 || correlation_coefficient < temp2)
			{
				parameters.copy(beta);
...
int MtLMFitting::calculate_lambda1(DMat& beta, double& lambda, double& s0, double& v)
{
	...
	_formula.calculate_sum_of_squares_of_deviations(*_delta, s1);
